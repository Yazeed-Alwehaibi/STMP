...
import { acceptApplication } from "../../controllers/supervisor/appdecision";
...
describe("acceptApplication", () => {
  ...
  it("accepts the application and responds with success", async () => {
    mockWhere.mockResolvedValue({ count: 1 });

    await acceptApplication(mockReq, mockRes);

    expect(mockUpdate).toHaveBeenCalledWith(applications);
    expect(mockRes.json).toHaveBeenCalledWith({ message: "Application accepted" });
  });

  it("returns 401 if user is missing", async () => {
    await acceptApplication({ ...mockReq, user: null }, mockRes);
