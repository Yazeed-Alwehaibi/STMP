  ...
  return (
    <>
      ...
      <div className="flex flex-col items-center mt-20">
            ...
            {usersTable && user && <p>Already logged in as {user.email}</p>}
            <form onSubmit={handleLogin} className="flex flex-col gap-3">
                <input className='bg-white' type="email" placeholder="Email" value={email} onChange={(e) => setEmail(e.target.value)} required />
                <input className='bg-white' type="password" placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} required />
